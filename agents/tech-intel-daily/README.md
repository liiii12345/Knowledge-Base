...
| 信源清单（5 个 YouTube 频道 + 23 个 Twitter/X 账号） | **真实**：均为公开账号，见 `config.example.yaml` |
...
- **真实采集链路未在本机验证**：`--demo` 用的是内置样例；真实采集依赖 YouTube 页面结构与 Nitter 镜像的可用性，两者都会随外部改版失效。仓库里能验证的是 `--demo` 全流程与单元测试，**不是** 28 个源的实时可用性
